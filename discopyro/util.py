...
def softmax_logits(logits, dim=-1):
    nonzeros = logits.nonzero(as_tuple=True)
    return torch.index_put(logits, nonzeros, F.softmax(logits[nonzeros],
                                                       dim=dim))

def batch_softmax(logprobs, dim1=-2, dim2=-1):
    logprobs = list(logprobs.unbind(dim=dim1))
    for i, logprob in enumerate(logprobs):
        logprobs[i] = softmax_logits(logprob, dim2)
...
def soften_logprobs(energies, temperature, dim1=-2, dim2=-1):
    if dim2 is None:
        return softmax_logits(energies / temperature, dim1)
